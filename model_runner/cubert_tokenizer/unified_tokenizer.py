...
def multi_token_from_token(token):
    return AbstractMultiToken(spellings=(token.spelling,),
                              kind=token.kind,
                              metadata=token.metadata, label=token.label)
  ...
  """
    ...
    while current_line < end.line:
        yield AbstractToken(
            ...
            TokenMetadata(
                ...
                end=Position(line=current_line + 1, column=0)), label=False)
        current_column = 0
        current_line += 1
    ...
    if number_of_final_spaces:
        ...
        yield AbstractToken(
            ...
            TokenMetadata(
                start=Position(line=current_line, column=current_column),
                end=Position(line=current_line, column=end.column)), label=False)
