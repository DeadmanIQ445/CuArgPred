...
class PythonTokenizer(cubert_tokenizer.CuBertTokenizer):
    ...
    def tokenize_and_abstract(self, source_code, labels):
        """Produces a language-agnostic tokenization of the input code.
        :param source_code:
        :param labels:
        """
        token_pairs: Iterable[Tuple[str, int, bool]]
        try:
            token_tuples = unified_tokenizer.code_to_tokens(source_code)
            token_pairs = ((token_name, token_type, 'def ' in line)
                           for token_type, token_name, _, _, line in token_tuples)
        ...
        for spelling, kind, first_line in token_pairs:
            ...
            if kind == tokenize.NAME:
                ...
                if kind in PythonTokenizer._TOKEN_TYPES_TO_TOKENIZE_BY_TYPE:
                    ...
                    adjusted_spelling = f'{cubert_tokenizer.token_from_token_type(kind)}{spelling}'
